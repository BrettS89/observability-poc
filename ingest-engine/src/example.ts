/**
 * Minimal Prometheus-scrape -> Prometheus remote_write -> Grafana Mimir
 *
 * - Scrapes /metrics text exposition
 * - Parses samples into TimeSeries
 * - Encodes WriteRequest protobuf
 * - Snappy-compresses payload
 * - POSTs to Mimir /api/v1/push with tenant header (X-Scope-OrgID)
 *
 * This is intentionally minimal but "real": it's the core data-plane architecture.
 */
...
// ----------------------------
// Prometheus remote_write proto (minimal)
// ----------------------------
// This is the canonical schema used by Prometheus remote write.
// We embed it so you don't need a .proto file.
...
// ----------------------------
// We parse only the sample lines; HELP/TYPE lines are ignored for remote_write.
// Supports:
//   metric_name 123
//   metric_name{a="b",c="d"} 123
//   metric_name{...} 123 1712345678901
//
// Notes:
// - Prometheus exposition allows NaN/+Inf/-Inf. We'll ignore non-finite values.
// - Labels are parsed with a minimal unescaper.
...
// ----------------------------
// Convert samples -> remote_write WriteRequest
// ----------------------------
